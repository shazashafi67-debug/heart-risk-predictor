...
if page == "🏃 Manual Lifestyle Input":
    ...
    active = st.selectbox("Regular Exercise", ["Yes", "No"])
    ...
    if st.button("Predict Heart Risk"):
        ...
        # Dummy dataset to train model (for demo)
    ...
    if uploaded_files:
        for file in uploaded_files:
            ...
            try:
                ...
                y_col = None  # No label expected; just dummy
                ...
                st.success(f"✅ Predictions done on `{file.name}` (dummy accuracy {acc:.2f})")
                df['Prediction'] = model.predict(X)

                st.subheader("Prediction Distribution")
                fig = px.bar(x=['Low Risk','High Risk'], y=[(df['Prediction']==0).sum(), (df['Prediction']==1).sum()])
